...
{
	if (fabs(Param::Vision::FRAME_RATE - 30) < 0.1) {
		_ballFilter.initialize(Param::File::BallFilterDir + "slowMatrices30.txt", Param::File::BallFilterDir + "fastMatrices30.txt");
	} else if (fabs(Param::Vision::FRAME_RATE - 60) < 0.1) {
		_ballFilter.initialize(Param::File::BallFilterDir + "slowMatrices60.txt", Param::File::BallFilterDir + "fastMatrices60.txt");
	} else {
		_ballFilter.initialize(Param::File::BallFilterDir + "slowMatrices71.txt", Param::File::BallFilterDir + "fastMatrices71.txt");
	}
...
void CBallPredictor::updateVision(int cycle, const PosT& raw_pos, bool invert)
{
		...
		for (int i=1; i<NORMAL_BALL_LOST_TIME*1.5; ++i) {
			if (_visionLogger.visionValid(cycle - i)) {
				for (int j=cycle-i; j<cycle; ++j) {
					...
					nextVision.SetRawPos(previousVision.RawPos() + previousVision.Vel() / Param::Vision::FRAME_RATE);
	...
	if (collisionRelate && checkValid(cycle)) {
		...
		const CVector ballVel(ballPosVel[0][1],ballPosVel[1][1]); // 球的速度(数组第二列)

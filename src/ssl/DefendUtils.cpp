...
namespace DefendUtils{
  ...
  CGeoEllipse RD_ELLIPSE1 = CGeoEllipse(CENTERPOINT, Param::Field::PENALTY_AREA_DEPTH + 15 + AVOIDBUFFER, (Param::Field::PENALTY_AREA_WIDTH + 50 + AVOIDBUFFER*2)/2);
  ...
  CGeoPoint DCENTERPOINT = CGeoPoint(-Param::Field::PITCH_LENGTH/2,0);
	...
	CGeoPoint calcGoaliePointV3(const CGeoPoint& RTarget, double RDir, posSide RSide, const CGeoPoint& lastpoint, int mode) {
		...
		if (up == true) {
			...
			if (RTarget.x() < Param::Field::MAX_PLAYER_SIZE)
				up = true;
		}
		...
		if (up == true) {
			...
			m = Param::Vehicle::V2::PLAYER_SIZE;
		...
		if (intersect.intersectant() == false || isTargetValid == false){
			if (POS_SIDE_LEFT == RSide) {
				double x = Param::Field::PITCH_LENGTH/2.0 - Param::Vehicle::V2::PLAYER_FRONT_TO_CENTER;
				double y = Param::Field::GOAL_WIDTH/2.0 - Param::Vehicle::V2::PLAYER_SIZE;

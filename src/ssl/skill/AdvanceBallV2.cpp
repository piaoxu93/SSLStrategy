...
namespace {
	...
	bool USE_INTERCEPT    = false;     // 是否使用截球
	//对方禁区信息
...
void CAdvanceBallV2::plan(const CVisionModule* pVision) {
		...
		case GOTO:
			if (pullBackFlag == true && isOppFaceTheirDoor(pVision, Param::Math::PI / 2) || (Utils::InTheirPenaltyArea(ball.Pos(), 0) && oppBlockMe(pVision, _executor))) {
				...
				if (meHasBall > meHasBallMaxCnt && oppBlockMe(pVision, _executor)) {
					...
					if (USE_DRIBBLE_KICK && !ballOnTheirHalf(pVision, 0)) { // 我方半场不能浪射
						_state = KICK_DRIBBLE;
					} else {
						_state = KICK;
					}
			...
			// 截球
			if (USE_INTERCEPT && ballMoving2Me(pVision, _executor) && ballOnTheirHalf(pVision, 0) && meNearballThanOpp(pVision, _executor)) {
				_state = INTERCEPT;
	...
	if (isToTheirGoal && ball.Pos().x() > 0 && _state!= NORMAL_PUSH && _state != ROTATE && _state != DRIBBLE_PULL && _state != STOP && _state != DRIBBLE_TURN && _state != LIGHT_KICK) {
	...
	case GOTO:
		if (Utils::InOurPenaltyArea(ball.Pos(), 0)) {
			...
			if (!ballOnTheirHalf(pVision)) { // 我方半场
				if (ball.Pos().x() > -130) {
					setSubTask(PlayerRole::makeItChaseKickV2(_executor, kickDir, flag_dodge_ball));
				}
				else {
					if (ball.Pos().x() < 0 && !meFaceTheirDoor(pVision, _executor, 2) && isOppFaceOurDoor(pVision, Param::Math::PI / 2.0) && (ball2oppDist < 60)) { // 我方车朝着自己半场，对手跟在我后面，此时应该赶紧转身把球清出边线，转身应该向内转
				...
				if (Utils::InTheirPenaltyArea(ball.Pos(), 0)) { // 球在对方禁区
					...
					else { // 一般都进到这里（对方半场 && 不在对方禁区 && (对方最有威胁车不面朝对方的球门 || 球不在我和对方最有威胁车之间）)
	...
	case INTERCEPT:
		KickDirection::Instance()->GenerateShootDir(_executor, pVision->OurPlayer(_executor).Pos());
		kickDir = KickDirection::Instance()->getRealKickDir();
		setSubTask((PlayerRole::makeItInterKickV2(_executor, kickDir, flag)));
		break;
	case NORMAL_PUSH:
		KickDirection::Instance()->GenerateShootDir(_executor, pVision->OurPlayer(_executor).Pos());
		if (!ballOnTheirHalf(pVision)) { // 我方半场
	...
	case KICK_DRIBBLE:
		...
		if (kick_dribble > 2 && is_dribble_kick_dir_ok) {
			KickStatus::Instance()->setChipKick(_executor, 999);  // 力度需要调整
		}
...
bool CAdvanceBallV2::ballMoving2Me(const CVisionModule* pVision, const int vecNumber) {
	...
	if (ball.Valid() && abs(diff_ballMoving2Me) < Param::Math::PI / 3) {
		return true;
	}
}

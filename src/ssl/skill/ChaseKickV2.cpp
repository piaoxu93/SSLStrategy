...
namespace
{
	...
	double crossWiseFactor[12] = { 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5 };
...
void CChaseKickV2::plan(const CVisionModule* pVision)
{
	...
	case SPEED_UP:
		...
		if (isVerticalBall){
			...
			speedUpDistanceY = fieldFactor*fabs(kickPos.y() - me.Pos().y()) * crossWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + max<double>(ballSpeed - 80, 0)*max<double>(sin(fabs(ballVelDir) - Param::Math::PI * 8 / 180), 0)*0.28;
			...
			speedUpDistanceX = fieldFactor*fabs(kickPos.x() - me.Pos().x()) * verticalWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + cos(fabs(ballVelDir))* max<double>(ballSpeed - 100, 0) * 0.25;
			...
			speedUpDistanceY = fieldFactor*fabs(kickPos.y() - me.Pos().y()) * crossWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + max<double>(ballSpeed - 80, 0)*max<double>(sin(fabs(ballVelDir) - Param::Math::PI * 2 / 180), 0)*0.40;
			speedUpDistanceY += speedupball_factorY*0.35;
			speedUpDistanceY = min(speedUpDistanceY, 58.0);
			speedUpDistanceY = max(speedUpDistanceY, 0.0);

			//speedUpDistanceX = 3 - sin(fabs(ballVelDir)) * 10 + kickPos.x() - me.Pos().x() + verticalWiseFactor[realNum] * Param::Vehicle::V2::PLAYER_SIZE;
			speedUpDistanceX = fieldFactor*fabs(kickPos.x() - me.Pos().x()) * verticalWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + cos(fabs(ballVelDir+Param::Math::PI * 10 / 180))* max<double>(ballSpeed - 100, 0) * 0.1;
			...
			speedUpDistanceY = fieldFactor*fabs(kickPos.y() - me.Pos().y()) * crossWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + max<double>(ballSpeed - 80, 0)*max<double>(sin(fabs(ballVelDir) - Param::Math::PI * 10 / 180), 0)*0.35;
			speedUpDistanceY += speedupball_factorY*0.3;
			speedUpDistanceY = min(speedUpDistanceY, 56.0);
			speedUpDistanceY = max(speedUpDistanceY, 0.0);
			//speedUpDistanceX = 3 - sin(fabs(ballVelDir)) * 10 + kickPos.x() - me.Pos().x() + verticalWiseFactor[realNum] * Param::Vehicle::V2::PLAYER_SIZE;
            speedUpDistanceX = fieldFactor*fabs(kickPos.x() - me.Pos().x()) * verticalWiseFactor[realNum-1] * Param::Vehicle::V2::PLAYER_SIZE + cos(fabs(ballVelDir+Param::Math::PI* 10/180))* max<double>(ballSpeed - 100, 0) * 0.15;

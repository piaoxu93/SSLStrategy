...
namespace{
	...
	const double STABLE_SLACK_TIME_BACK = 0.6;
...
void CInterceptBallV2::plan(const CVisionModule* pVision){
	...
	bool isNeedGetBall = ballSpeed<BALL_SLOW_SPEED;
	...
	if (fabs(Utils::Normalize(meDir - finalKickDir)) < math_onedir * 2){
		_normalKickCounter++;
	}
	else{
		_normalKickCounter = 0;
	}
	if (_normalKickCounter == 3){
		canNormalKick = true;
	}
	...
	switch (state()){
		...
		case RUSH_TO_BALL:
			...
			if (slackTime > stableSlackTime+0.2&&distToBall>distSub){
					targetPoint = projPoint + Utils::Polar2Vector(max<double>((250-ballSpeed),0)*ballSpeedFactor, antiBallVelDir);    //TODO 关键在于如何使得迭代结果收敛于slackTime=0的点
			}
		...
		case STABLE_BALL:
			...
			if (canNormalKick){
				if (isNeedChipKick){
					...
					_drribleKickCounter++;
					if (_drribleKickCounter == 10){
						KickStatus::Instance()->setChipKick(robotNum, kickPower);
						cout << "Chip" << endl;
					}
				}
				else{
			...
			else if (ballSpeed < 30 && distToBall < ALLOWANCE_DIST_TO_STABLE){

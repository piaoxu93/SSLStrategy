...
void CShootBall::plan(const CVisionModule* pVision)
{
	...
	const CGeoSegment chaseTargetSeg = CGeoSegment(CGeoPoint(Param::Field::PITCH_LENGTH / 2, -200), CGeoPoint(Param::Field::PITCH_LENGTH / 2, 200));
	...
	bool isBallSlow = ballSpeed < 50;
	bool isNeedTouchKick = ballSpeed > 120 && ballSpeed<280 && a_m2t_2_m2b < 35 && a_m2t_2_bsr<35 && self2ball.mod()>100 && a_m2b_2_bsr<20 && ball2projDist / projDist>2 && projDist < 75;
	...
	bool isNeedInterKickForCrossBall = fabs(ballVelDir) >= 80 && fabs(ballVelDir) <= 100 && self2ball.mod()>200&&meOnBallMovingSeg;
	...
	int new_state = state(), old_state = state();
		...
		switch (old_state){
			...
			case CHASE_KICK:
				...
				break;
			case INTERCEPT_KICK:
				if (fabs(ballVelDir) <= math_onedir * 80 && chaseLineFlag&&!isNeedInterKickForCrossBall&&self.Pos().x() - ball.Pos().x() <= 200&&ballSpeed>150&&projDist>50 ||ball.Pos().x()>300&&fabs(ballVelDir)<math_onedir*100||ballSpeed<BALL_SLOW_SPEED){
	...
	setState(new_state);
	if(verBos&&false) cout<<"shoot"<<new_state<<endl;
	if(_out_inter){
		_out_inter_cnt++;
	...
	switch (state()){
		...
		case INTERCEPT_KICK:
			{
				setSubTask(PlayerRole::makeItInterKickV2(runner,finalDir,flags));
			}
			break;
		case DEFEND_GOAL:
			{
	...
	if (is_runner_dir_ok || NormalPlayUtils::faceTheirGoal(pVision,runner)) {
		if (!task().player.ischipkick) {
			if (state()==TOUCH_KICK)
			KickStatus::Instance()->setKick(runner,900);
			else
